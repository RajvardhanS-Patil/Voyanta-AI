...
class WeatherIntelligenceEngine {
  List<TravelRecommendation> analyze(JourneyState journeyState, String weatherInfo) {
    ...
    if (info.contains('rain') || info.contains('storm') || info.contains('shower')) {
      recommendations.add(
        const TravelRecommendation(
          ...
          title: 'Rain Warning',
          description: 'Rain showers detected. Consider swapping to indoor museum exhibits early.',
          actionLabel: 'Adjust Plan',
        ),
      );
    } else if (info.contains('hot') || info.contains('heat') || info.contains('sunny')) {
      recommendations.add(
        const TravelRecommendation(
          ...
          description: 'High UV index afternoon. Stay hydrated and plan indoor rest breaks.',
        ),
      );
    }
